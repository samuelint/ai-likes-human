...
impl Builder {
    pub fn create_with_backend(backend: Arc<LlamaBackend>) -> Self {
        let model_factory = Arc::new(CachedModelFactory::new(
            backend.clone(),
    ...
    pub fn with_model(self, model_path: &str, options: ModelOptions) -> Result<Self> {
        self.model_factory.load_model(model_path, &options)?;
