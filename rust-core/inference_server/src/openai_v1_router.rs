...
use crate::{
    api::{
        assistant_thread::{create_thread, create_thread_message, create_thread_run, delete_thread, delete_thread_message, find_thread, find_thread_message, find_thread_run, list_thread_messages, list_thread_run, list_threads, update_thread},
...
fn create_thread_router(state: Arc<ServerState>) -> Router<Arc<ServerState>> {
    Router::new()
        ...
        .route("/:thread_id/messages/:message_id", delete(delete_thread_message))
        ...
        .route("/:thread_id/runs", get(list_thread_run))
        .with_state(Arc::clone(&state))
}
