...
async fn create_thread_and_run_stream(
    prompt: &str,
    ...
    client
        .stream_create_thread_and_run(&create_thread_run_dto)
        .await
}

async fn stream_as_chunks_array(prompt: &str) -> Vec<ThreadEvent> {
    let mut stream = create_thread_and_run_stream(prompt).await;
...
#[tokio::test]
async fn test_stream_create_thread_and_run_single_step_is_sequenced_by_events() {
